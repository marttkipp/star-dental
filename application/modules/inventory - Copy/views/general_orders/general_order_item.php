    	...
    	<div class="row">
			<div class="col-md-12">
				<div class="center-align">
					...
					{
						?>
							<a class="btn btn-success btn-sm" href="<?php echo base_url();?>inventory/send-general-order-for-approval/<?php echo $order_id;?>/<?php echo $next_order_status;?>" onclick="return confirm('Do you want to complete the order?');">Complete Order</a>
						<?php
					}
					...
					<div class="col-md-12">
						...
						foreach($order_item_query->result() as $res)
						{
		                    ...
		                    // var_dump($order_approval_status); die();
								if(($order_approval_status == 0))
								{
				                    $result .= ' '.form_open('inventory/update-general-order-item/'.$order_id.'/'.$order_number.'/'.$order_item_id).'
												<tr>
												<input type="hidden" class="form-control" name="product_id" value="'.$product_id.'">
													...
													<td><button class="btn btn-success btn-sm" type="submit"><i class="fa fa-pencil"></i> Edit Item</button></td>
													<td><a href="'.site_url().'inventory/delete-general-order-item/'.$order_item_id.'/'.$order_id.'/'.$order_number.'" onclick="return confirm("Do you want to delete '.$product_name.'?")" title="Delete '.$product_name.' class="btn btn-danger btn-sm">Delete</a></td>
												</tr>
												'.form_close().'
												';
								}
