...
class Orders extends MX_Controller
{
	...
	*/
    ...
    public function add_order_item($order_id,$order_number)
    {
		...
		// var_dump($v_data['suppliers_query']);die();
  ...
  //       $in_stock = ($quantity + $purchases + $procurred) - $sales - $deductions;
	...
	*/
	...
	public function search_supplier_credit_notes()
	{
		...
		if(!empty($invoice_number) )
		{
			$invoice_number = ' AND (orders.supplier_invoice_number LIKE \'%'.mysql_real_escape_string($invoice_number).'%\' OR orders.reference_number LIKE \'%'.mysql_real_escape_string($invoice_number).'%\')';
		}


		if(!empty($credit_note_number))
		{
			$credit_note_number = ' AND (orders.supplier_invoice_number LIKE \'%'.mysql_real_escape_string($credit_note_number).'%\' OR  orders.reference_number LIKE \'%'.mysql_real_escape_string($credit_note_number).'%\')';
		}
		// var_dump($supplier_id); die();
	    if(!empty($supplier_id))
        {
