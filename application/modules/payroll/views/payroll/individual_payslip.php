						...
						$total_net = 0;
							...
							$result .= '<div class="row" >
								        	...
								        	<div class="col-md-12 center-align receipt_bottom_border">
								            	<strong>Insight Management Consultants LTD<br/>
												...
												'<div class="row">';
										                    ...
										                    if($payments->num_rows() > 0)
															{
																foreach($payments->result() as $res)
																{
																	...
																	$payment_amt = $this->payroll_model->get_payroll_amount($personnel_id, $payroll_items, $payment_table, $table_id);
												    ...
												    '<div class="row">';
														...
														if($benefits->num_rows() > 0)
														{
															foreach($benefits->result() as $res)
															{
																...
																$benefit_amt = $this->payroll_model->get_payroll_amount($personnel_id, $payroll_items, $benefit_table, $table_id);
												    ...
												    '<div class="row">';
														...
														if($allowances->num_rows() > 0)
														{
															foreach($allowances->result() as $res)
															{
																...
																$allowance_amt = $this->payroll_model->get_payroll_amount($personnel_id, $payroll_items, $allowance_table, $table_id);
														...
														//nhif
														$nhif = $this->payroll_model->get_payroll_amount($personnel_id, $payroll_items, $nhif_table, 1);
														$total_nhif += $nhif;
														
														//paye
														$paye =$this->payroll_model->get_payroll_amount($personnel_id, $payroll_items, $paye_table, 1);
														
														//relief
														$relief = $this->payroll_model->get_payroll_amount($personnel_id, $payroll_items, $relief_table, 1);
														
														//insurance_relief
														$insurance_relief = $this->payroll_model->get_payroll_amount($personnel_id, $payroll_items, $insurance_relief_table, 1);
														
														//relief
														$insurance_amount = $this->payroll_model->get_payroll_amount($personnel_id, $payroll_items, $insurance_amount_table, 1);
					                				...
					                				'<div class="row">
														...
														if($deductions->num_rows() > 0)
														{
															foreach($deductions->result() as $res)
															{
																...
																$deduction_amt = $this->payroll_model->get_payroll_amount($personnel_id, $payroll_items, $deduction_table, $table_id);
														...
														if($other_deductions->num_rows() > 0)
														{
															foreach($other_deductions->result() as $res)
															{
																...
																$other_deduction_amt = $this->payroll_model->get_payroll_amount($personnel_id, $payroll_items, $other_deduction_table, $table_id);
