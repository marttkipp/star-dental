...
if ($query->num_rows() > 0)
{
	...
	$total_personnel_overtime = $total_personnel_benefits = $total_personnel_payments = $total_personnel_allowances = $total_personnel_deductions = $total_personnel_other_deductions = $total_personnel_loans = array();
	...
	foreach ($query->result() as $row)
	{
		...
		$personnel_total_net = 0;// to check if we have 0 net and loans
		...
		if($payments->num_rows() > 0)
		{
			foreach($payments->result() as $res)
			{
				...
				if($total_payment_amount[$payment_id] != 0)
				{
					...
					$personnel_total_net += $payment_amt;
				}
			}
		}
		...
		if($allowances->num_rows() > 0)
		{
			foreach($allowances->result() as $res)
			{
				...
				if($total_allowance_amount[$allowance_id] != 0)
				{
					...
					$personnel_total_net += $allowance_amt;
				}
			}
		}
		...
		if($overtime->num_rows() > 0)
		{
			foreach($overtime->result() as $res)
			{
				...
				if($total_overtime_amount[$overtime_id] != 0)
				{
					...
					$personnel_total_net += $overtime_amt;
				}
			}
		}
		...
		if($deductions->num_rows() > 0)
		{
			foreach($deductions->result() as $res)
			{
				...
				if($total_deduction_amount[$deduction_id] != 0)
				{
					...
					$personnel_total_net -= $deduction_amt;
				}
			}
		}
		...
		if($other_deductions->num_rows() > 0)
		{
			foreach($other_deductions->result() as $res)
			{
				...
				if($total_other_deduction_amount[$other_deduction_id] != 0)
				{
					...
					$personnel_total_net -= $other_deduction_amt;
				}
			}
		}
		...
		if($rs_schemes->num_rows() > 0)
		{
			foreach($rs_schemes->result() as $res)
			{
				...
				if($total_loan_scheme_amount[$loan_scheme_id] != 0)
				{
					...
					if(!isset($total_personnel_loans[$loan_scheme_id]))
					{
						$total_personnel_loans[$loan_scheme_id] = 0;
					}
					
					$all_loans_total[$loan_scheme_id] += $loan_scheme_amt;
					$total_personnel_loans[$loan_scheme_id] += $loan_scheme_amt;
				}
			}
		}
	}
	...
	if($payments->num_rows() > 0)
	{
		foreach($payments->result() as $res)
		{
			...
			if($total_payment_amount[$payment_id] != 0)
			{
				$gross += $total_personnel_payments[$payment_id];
				$result .= '
				<tr>
					<th>'.$payment_abbr.'</th>
					<th>'.number_format($total_personnel_payments[$payment_id], 2, '.', ',').'</th>
				</tr>';
			}
		}
	}
	...
	if($benefits->num_rows() > 0)
	{
		foreach($benefits->result() as $res)
		{
			...
			if($total_benefit_amount[$benefit_id] != 0)
			{
				$gross += $total_personnel_benefits[$benefit_id];
				$result .= '
				<tr>
					<th>'.$benefit_abbr.'</th>
					<th>'.number_format($total_personnel_benefits[$benefit_id], 2, '.', ',').'</th>
				</tr>';
			}
		}
	}
	...
	if($allowances->num_rows() > 0)
	{
		foreach($allowances->result() as $res)
		{
			...
			if($total_allowance_amount[$allowance_id] != 0)
			{
				$gross += $total_personnel_allowances[$allowance_id];
				$result .= '
				<tr>
					<th>'.$allowance_abbr.'</th>
					<th>'.number_format($total_personnel_allowances[$allowance_id], 2, '.', ',').'</th>
				</tr>';
			}
		}
	}
	...
	if($overtime->num_rows() > 0)
	{
		foreach($overtime->result() as $res)
		{
			...
			if($total_overtime_amount[$overtime_id] != 0)
			{
				$gross += $total_personnel_overtime[$overtime_id];
				$result .= '
				<tr>
					<th>'.$overtime_name.'</th>
					<th>'.number_format($total_personnel_overtime[$overtime_id], 2, '.', ',').'</th>
				</tr>';
			}
		}
	}
	...
	$net = $gross - ($total_paye + $total_nssf + $total_nhif);
	//deductions
	if($deductions->num_rows() > 0)
	{
		foreach($deductions->result() as $res)
		{
			...
			if($total_personnel_deductions[$deduction_id] != 0)
			{
				$net -= $total_personnel_deductions[$deduction_id];
				$result .= '
				<tr>
					<th>'.$deduction_name.'</th>
					<th>'.number_format($total_personnel_deductions[$deduction_id], 2, '.', ',').'</th>
				</tr>';
			}
		}
	}
	...
	if($other_deductions->num_rows() > 0)
	{
		foreach($other_deductions->result() as $res)
		{
			...
			if($total_personnel_other_deductions[$other_deduction_id] != 0)
			{
				$net -= $total_personnel_other_deductions[$other_deduction_id];
				$result .= '
				<tr>
					<th>'.$other_deduction_abbr.'</th>
					<th>'.number_format($total_personnel_other_deductions[$other_deduction_id], 2, '.', ',').'</th>
				</tr>';
			}
		}
	}
	...
	if($rs_schemes->num_rows() > 0)
	{
		foreach($rs_schemes->result() as $res)
		{
			...
			if($total_personnel_loans[$loan_scheme_id] != 0)
			{
				$net -= $total_personnel_loans[$loan_scheme_id];
				$result .= '
				<tr>
					<th>'.$loan_scheme_name.'</th>
					<th>'.number_format($total_personnel_loans[$loan_scheme_id], 2, '.', ',').'</th>
				</tr>';
			}
		}
	}
