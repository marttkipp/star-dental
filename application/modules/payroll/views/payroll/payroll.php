...
if ($query->num_rows() > 0)
{
	...
	if($savings_rs->num_rows() > 0)
	{
		...
		{
			$savings_abbr = $res->savings_name;
			$savings_id = $res->savings_id;
				
			$total_savings_amount[$savings_id] = 0;

			// var_dump($savings->$savings_id);die();
			if(isset($savings->$savings_id))
			{
				$total_savings_amount[$savings_id] = $savings->$savings_id;
			}
			
			if($total_savings_amount[$savings_id] != 0)
			{
				$result .= '<th>'.$savings_abbr.'</th>';
			}
		}
	}
	...
	foreach ($query->result() as $row)
	{
		...
		$id_number = $row->personnel_national_id_number;
		...
		if($overtime->num_rows() > 0)
		{
			foreach($overtime->result() as $res)
			{
				...
				if($total_overtime_amount[$overtime_id] != 0)
				{
					...
					$result .= '<td>'.number_format($overtime_amt, 2).'</td>';
				}
			}
		}
		...
		*/
		...
		// savings
		$total_savings = 0;
		// var_dump($savings_rs->num_rows()); die();
		if($savings_rs->num_rows() > 0)
		{
			foreach($savings_rs->result() as $res)
			{
				...
				if($total_savings_amount[$savings_id] != 0)
				{
					...
					// var_dump($savings_amt); die();
					$total_savings += $savings_amt;
					if(!isset($total_personnel_savings[$savings_id]))
					{
						$total_personnel_savings[$savings_id] = 0;
					}
					$total_personnel_savings[$savings_id] += $savings_amt;
					$result .= '<td>'.number_format($savings_amt, 2).'</td>';
				}
			}
		}
		...
		if($rs_schemes->num_rows() > 0)
		{
			foreach($rs_schemes->result() as $res)
			{
				...
				if($total_loan_scheme_amount[$loan_scheme_id] != 0)
				{
					...
					$result .= '<td>'.number_format($loan_scheme_amt, 2).'</td>';
				}
			}
		}
		/*$date = date("Y-m-d");
		...
		//net
		$net = $gross - ($paye + $nssf + $nhif + $total_deductions + $total_other_deductions + $total_schemes + $total_savings);
		$total_net += $net;
	...
	$result .= '
			<tr> 
				<td colspan="'.($total_number_of_payments-4).'"></td>';
	if($payments->num_rows() > 0)
	{
		foreach($payments->result() as $res)
		{
	...
	if($overtime->num_rows() > 0)
	{
		foreach($overtime->result() as $res)
		{
			...
			if($total_overtime_amount[$overtime_id] != 0)
			{
				$result .= '<th>'.number_format($total_personnel_overtime[$overtime_id], 2).'</th>';
			}
		}
	}
	//gross
	...
	if($savings_rs->num_rows() > 0)
	{
		foreach($savings_rs->result() as $res)
		{
			...
			if($total_savings_amount[$savings_id] != 0)
			{
				$result .= '<th>'.number_format($total_personnel_savings[$savings_id], 2).'</th>';
			}
		}
	}
	...
	if($rs_schemes->num_rows() > 0)
	{
		foreach($rs_schemes->result() as $res)
		{
			...
			if($all_loans_total[$loan_scheme_id] != 0)
			{
				$result .= '<th>'.number_format($all_loans_total[$loan_scheme_id], 2).'</th>';
			}
		}
	}
