<!-- search -->
<?php //echo $this->load->view('search/search_lender_account', '', TRUE);?>
<!-- end search -->
    ...
    <div class="col-md-12">
        ...
        <section class="panel">
            <header class="panel-heading">
                ...
                <a href="<?php echo site_url();?>accounting/lenders" class="btn btn-sm btn-warning pull-right" style="margin-top: -25px; "><i class="fa fa-arrow-left"></i> Back to lenders</a>
                <a href="<?php echo base_url().'financials/company_financial/print_lender_statement/'.$lender_id?>" class="btn btn-sm btn-success pull-right"  style="margin-top: -25px;margin-right: 5px;" target="_blank"><i class="fa fa-print"></i> Print</a>
                <a href="<?php echo site_url().'search-lender-bill/'.$lender_id?>" class="btn btn-sm btn-primary pull-right"  style="margin-top: -25px; margin-right: 5px;"><i class="fa fa-plus"></i> Add Bill</a>
                <a href="<?php echo site_url().'search-lender-credit-notes/'.$lender_id?>" class="btn btn-sm btn-danger pull-right"   style="margin-top: -25px; margin-right: 5px;"><i class="fa fa-plus"></i> Add Credit Note</a>
                <a  href="<?php echo site_url().'search-lender-payments/'.$lender_id?>" class="btn btn-sm btn-primary pull-right"   style="margin-top: -25px; margin-right: 5px;"><i class="fa fa-plus"></i> Add Payment</a>
            ...
            <div class="panel-body">
                <div class="pull-right">
                	...
                	<!--<a href="<?php echo base_url().'administration/sync_app_lender_account';?>" class="btn btn-sm btn-info"><i class="fa fa-sign-out"></i> Sync</a>-->
                </div>
                <!-- Modal -->
			...
			{
				echo '
				<a href="'.site_url().'financials/company_financial/close_lender_expense_ledger/'.$lender_id.'" class="btn btn-warning btn-sm ">Close Search</a>
				';
				echo $search_title;
			}

			// var_dump($lender_id);die();
				$opening_balance_rs = $this->company_financial_model->get_lender_statement_balance($lender_id);
				$lender_result = $this->company_financial_model->get_lender_statement($lender_id);
			...
			?>
              ...
              $button = '';
				  		if($lender_result->num_rows() > 0)
				  		{
				  			...
				  			foreach ($lender_result->result() as $key => $value) {
				  				# code...
				  				...
				  				if($transactionClassification === "Lenders Invoice")
				  				{
				  					// var_dump($transactionClassification);die();
				                    // if($transactionClassification === "Lenders Invoice")
				                    // {
				                    //   $button =  '';
				                    // }
				                    // else {
				                      $button = '<td><a href="'.base_url().'goods-transfered-notes/'.$referenceId.'" class="btn btn-xs btn-success" target="_blank"> View Transfer </a></td>';

				                    // }
				                    $amount_paid = $this->company_financial_model->get_lender_amount_paid($transactionId,$recepientId);
                  				...
                  				if($transactionClassification == "Lenders Credit Note")
				  				{
				  				...
				  				if($transactionClassification == "Lenders Invoices")
				  				{
                    				$button = '';
                    				$amount_paid = $this->company_financial_model->get_lender_amount_paid($transactionId,$recepientId);
				  				...
				  				if($transactionClassification == 'Lenders Invoices Payments')
				  				{
				  				...
				  				if($transactionClassification == 'Lenders Credit Notes')
				  				{
				  				...
				  				{
                    				$button = '<td><a href="'.site_url().'allocate-payment/'.$transactionId.'/'.$transactionId.'/'.$lender_id.'" class="btn btn-xs btn-danger"> Allocate payment </a></td>';

				  				}



				  				if($transactionClassification == 'Lender Opening Balance')
				  				{
				  					$result .= '<tr>
...
<script type="text/javascript">
    ...
    function get_invoice_details(reference_id,type)
    {
		...
		var url = "<?php echo site_url();?>finance/lenders/get_invoice_details/"+reference_id;
		// alert(url);
    ...
    function get_payment_details(reference_id,type)
    {
		...
		var config_url = $('#config_url').val();
		var url = "<?php echo site_url();?>finance/lenders/get_payment_details/"+reference_id;
		// alert(url);
    ...
    function get_credit_note_details(reference_id,type)
    {
		...
		var config_url = $('#config_url').val();
		var url = "<?php echo site_url();?>finance/lenders/get_credit_note_details/"+reference_id;
		// alert(url);
    ...
    function get_supplier_details(reference_id,type)
    {
		...
		var config_url = $('#config_url').val();
		var url = "<?php echo site_url();?>finance/lenders/get_suppliers_invoice_details/"+reference_id;
		// alert(url);
