...
$balance_rs = $this->company_financial_model->get_product_balance_brought_forward();

if($balance_rs->num_rows() > 0)
{
  foreach ($balance_rs->result() as $key => $value_rs) {
    // code...
    $dr_quantity = $value_rs->dr_quantity;
    $cr_quantity = $value_rs->cr_quantity;
    $dr_amount = $value_rs->dr_amount;
    $cr_amount = $value_rs->cr_amount;
...
if($query->num_rows() > 0)
{
	...
	$result .='
							<tr>
										...
										<th class="text-center" style="border-top:#3c8dbc solid 1px;">'.$total_dr_quantity.'</th>
                    <th class="text-center" style="border-top:#3c8dbc solid 1px;">'.$total_cr_quantity.'</th>
										<th class="text-center" style="border-top:#3c8dbc solid 1px;">'.number_format($total_dr_amount,2).'</th>
                    <th class="text-center" style="border-top:#3c8dbc solid 1px;">'.number_format($total_cr_amount,2).'</th>
							</tr>
					';
$current_stock = $total_dr_quantity - $total_cr_quantity;
$result .='
            <tr>
                  <th class="text-left"></th>
                  <th class="text-left"></th>
                  ...
                  <th class="text-center" colspan="2" style="border-top:#3c8dbc solid 2px;">'.$current_stock.'</th>
                  <th class="text-center" colspan="2" style="border-top:#3c8dbc solid 2px;">'.number_format($total_dr_amount - $total_cr_amount,2).'</th>
            </tr>
        ';
}
?>
    	...
    	<table class="table  table-striped table-condensed">
			<thead>
    			...
    			<th class="text-left">Description</th>
						<th class="text-center">Dr QTY</th>
          <th class="text-center">Cr QTY</th>
						<th class="text-center">Dr value</th>
        	<th class="text-center">Cr Value</th>
				</tr>
			</thead>
			<tbody>
						<?php echo $result?>
