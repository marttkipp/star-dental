...
class Departments extends Hospital_administration 
{
	...
	*/
	...
	public function department_accounts($department_id)
	{
		...
		{		
			...
			if($this->departments_model->add_account_to_department($department_id))
			{
				$this->session->set_userdata('success_message', 'Promotion has been added successfully');
		...
		$where = 'department_account.account_id = account.account_id AND department_id = '.$department_id;
		$table = 'department_account,account';
		//pagination
		$segment = 4;
		$this->load->library('pagination');
		...
		$page = ($this->uri->segment($segment)) ? $this->uri->segment($segment) : 0;
        $v_data["links"] = $this->pagination->create_links();
		$query = $this->departments_model->get_all_departments_account($table, $where, $config["per_page"], $page);
		
		
		$data['title'] = 'Department Account';
		$v_data['title'] = $data['title'];
		$v_data['department_id'] = $department_id;
		$v_data['query'] = $query;
		$v_data['page'] = $page;
		$data['content'] = $this->load->view('departments/all_department_accounts', $v_data, true);
