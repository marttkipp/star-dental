<?php
 ...
 $expense_accounts = $this->purchases_model->get_child_accounts("Expense Accounts");
  ...
  $lender_invoice_details = $this->lenders_model->get_lender_credit_note_details($lender_credit_note_id);
...
<div class="row">
  <section class="panel">
      ...
      <div class="panel-body">
        <?php echo form_open("finance/lenders/add_credit_note_item/".$lender_id.'/'.$lender_credit_note_id, array("class" => "form-horizontal"));?>
      ...
      <?php echo form_open("finance/lenders/confirm_credit_note/".$lender_id.'/'.$lender_credit_note_id, array("class" => "form-horizontal"));?>
      <?php
      $invoice_where = 'lender_credit_note_item.lender_id = '.$lender_id.' AND lender_credit_note_item.account_to_id = account.account_id AND lender_credit_note_id = '.$lender_credit_note_id;
      ...
      if($invoice_query->num_rows() > 0)
      {
        ...
        foreach ($invoice_query->result() as $key => $value) {
          ...
          $result_payment .= '<tr>
                                  ...
                                  <td><a href="'.site_url().'delete-credit-note-item/'.$lender_credit_note_item_id.'/'.$lender_credit_note_id.'" type="submit" class="btn btn-sm btn-danger" ><i class="fa fa-trash"></i></a></td>
                              </tr>';
                      ...
                      <select class="form-control  select2" name="invoice_id" id="invoice_id" required>
                        ...
                        if($lender_invoices->num_rows() > 0)
                        {
                          foreach ($lender_invoices->result() as $key => $value) {
                            // code...
                            $invoice_id = $value->lender_invoice_id;
                            $invoice_number = $value->invoice_number;

                            if($lender_invoice_id == $invoice_id)
                            {
                            	echo '<option value="'.$lender_invoice_id.'" selected> '.$invoice_number.'</option>';	
                            }
                            else
                            {
