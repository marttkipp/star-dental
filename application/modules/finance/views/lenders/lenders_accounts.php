<?php echo $this->load->view('search/search_lenders_account', '', TRUE);?>

<?php

$income_rs = $this->company_financial_model->get_lender_aging_report();
$income_result = '';
$total_income = 0;
if($income_rs->num_rows() > 0)
{
	foreach ($income_rs->result() as $key => $value) {
		...
		$lenders_id = $value->recepientId;
		$Total = $value->Total;
		$total_unallocated = $this->company_financial_model->get_unallocated_funds($lenders_id);
		$income_result .='<tr>
							...
							<td class="text-right"><a href="'.site_url().'lender-statement/'.$lenders_id.'" class="btn btn-warning btn-xs"> Open Account</a></td>
							<td class="text-right"><a href="'.site_url().'finance/edit-lender/'.$lenders_id.'" class="btn btn-success btn-xs"> Edit Account</a></td>
							</tr>';
	}
...
<section class="panel">
		<header class="panel-heading">
				...
				<div class="box-tools pull-right">
                    <a href="<?php echo site_url();?>finance/add-lender" class="btn btn-sm btn-primary" ><i class="fa fa-plus"></i> Add lenders</a>
                </div>
		</header>
		<div class="panel-body">
		<?php
      		$search = $this->session->userdata('lenders_search');
			if(!empty($search))
			{
				?>
                <a href="<?php echo base_url().'finance/lenders/close_lenders_lenders_search';?>" class="btn btn-sm btn-success"><i class="fa fa-print"></i> Close Search</a>
                <?php
			}
			?>
