...
<div class="row" style="margin-top: 10px;">
	<div class="col-md-6">
		<?php echo form_open("finance/budget/search_budget", array("class" => "form-horizontal"));?>
		<div class="col-md-6">
		...
		<div class="col-md-2">
			<?php
			$budget_year_searched = $this->session->userdata('budget_year');

			if(!empty($budget_year_searched))
			{
				?>
				<a href="<?php echo site_url().'finance/budget/close_budget_search'?>" class="btn btn-sm btn-warning"> CLOSE</a>
				<?php
			}
	...
	<div class="col-md-2">
		<a onclick="javascript:xport.toCSV('testTable');" class="btn btn-sm btn-success"> Export to excel</a>
	</div>
</div>
<div class="row">
...
<script type="text/javascript">
	...
	function get_year_budget(budget_year)
	{
		var config_url = $('#config_url').val();
	 	var url = config_url+"finance/budget/get_year_budget/"+budget_year;
	 	// alert(url);
		$.ajax({
			type:'POST',
			url: url,
	...
	function add_budget_item(budget_year)
	{
		...
		var config_url = $('#config_url').val();
		var data_url = config_url+"finance/budget/add_budget_item/"+budget_year;
	...
	$(document).on("submit","form#confirm-budget-item",function(e)
	{
		...
		var url = config_url+"finance/budget/confirm_budget_item/"+budget_year;
	...
	function delete_budget_item(budget_item_id,budget_year,month,account_id)
	{
		...
		{
			var config_url = $('#config_url').val();
			var data_url = config_url+"finance/budget/delete_budget_item/"+budget_item_id;
			//window.alert(data_url);
			$.ajax({
			type:'POST',
			url: data_url,
	...
	function edit_budget_item(month,account_id,budget_year)
	{
		...
		var config_url = $('#config_url').val();
		var data_url = config_url+"finance/budget/add_budget_item/"+budget_year+"/"+month+"/"+account_id;
		// window.alert(data_url);
		$.ajax({
		type:'POST',
		url: data_url,
	...
	function get_budget_items(budget_year,month,account_id)
	{
		...
		var config_url = $('#config_url').val();
	 	var url = config_url+"finance/budget/get_budget_list/"+budget_year+"/"+month+"/"+account_id;
	 	// alert(url);
		$.ajax({
			type:'POST',
			url: url,
