...
<div class="col-md-3">
  <section class="panel">
      ...
      <a href="<?php echo site_url().'creditor-statement/'.$creditor_id;?>" class="btn btn-warning btn-block"><b><i class="fa fa-arrow-left"></i> Back to statement</b></a>
    </div>
    <!-- /.box-body -->
      </div>
  </section>
...
<div class="col-md-9">
  <section class="panel">
    ...
    <?php echo form_open("finance/creditors/confirm_payment/".$creditor_id."/".$creditor_payment_id, array("class" => "form-horizontal"));?>
      ...
      <?php
      $invoice_where = 'creditor_payment_item.creditor_id = '.$creditor_id.' AND creditor_payment_item.creditor_payment_id = '.$creditor_payment_id;
      ...
      if($invoice_query->num_rows() > 0)
      {
        ...
        foreach ($invoice_query->result() as $key => $value) {
          // code...
          $creditor_payment_item_id_db = $value->creditor_payment_item_id;
          ...
          $result_payment .= '<tr class="'.$color.'">
                                  ...
                                  <td><a href="'.site_url().'delete-payment-item/'.$creditor_payment_id.'/'.$creditor_payment_item_id.'/'.$creditor_payment_item_id_db.'/'.$creditor_id.'" onclick="return confirm("Do you want to remove this entry ? ")" type="submit" class="btn btn-sm btn-danger" ><i class="fa fa-trash"></i></a></td>
                              </tr>';
        }
      ...
      if($display)
      {
      	...
      	$creditor_payment_where = 'creditor_payment_id = '.$creditor_payment_id;
		$creditor_payment_table = 'creditor_payment';
		$creditor_payment_order = 'creditor_payment_id';

		$creditor_payment_query = $this->creditors_model->get_creditors_list($creditor_payment_table, $creditor_payment_where, $creditor_payment_order);

		$creditor_payment_rs = $creditor_payment_query->row();
		$account_from_id = $creditor_payment_rs->account_from_id;
		$reference_number = $creditor_payment_rs->reference_number;
		$total_amount = $creditor_payment_rs->total_amount;
		$transaction_date = $creditor_payment_rs->transaction_date;
    ...
    <section class="panel">
        ...
        <div class="panel-body">
            <table class="table table-hover table-bordered col-md-12">
              ...
              <tbody>
                <?php
                	$creditor_invoices = $this->creditors_model->get_creditor_payments($creditor_id,10);
                ...
                if($creditor_invoices->num_rows() > 0)
                {
                  ...
                  foreach ($creditor_invoices->result() as $key) {
                    ...
                    <tr>
                      ...
                      <!-- <td><a href="<?php echo site_url().'cash-office/print-invoice-note/'.$creditor_payment_id.'/'.$creditor_id;?>" class="btn btn-sm btn-primary" target="_blank">Credit Note</a></td> -->
