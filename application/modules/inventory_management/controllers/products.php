...
class Products extends MX_Controller  
{
	...
	*/
	...
	{
		$where = 'product.category_id = category.category_id AND product.creditor_id = creditor.creditor_id AND product.product_deleted = 0';
	...
	public function add_product_details()
	{
		...
		$this->form_validation->set_rules('creditor_id', 'Supplier Name', 'required|xss_clean');
	...
	*/
	public function edit_product($product_id) 
	{
		...
		$query = $this->products_model->get_supplier($product_id);
	...
	*/
	...
	{
		//export products template in excel 
		 $this->products_model->opening_balance_import_template();
	}
	...
	function import_balances()
	{
		//open the add new product
		$v_data['title'] = $data['title'] = 'Import Balances';
		$data['content'] = $this->load->view('products/import_balances', $v_data, true);
		$this->load->view('admin/templates/general_page', $data);
	}
	
	function do_balances_import()
	{
		if(isset($_FILES['import_csv']))
		{
			if(is_uploaded_file($_FILES['import_csv']['tmp_name']))
			{
				//import products from excel 
				$response = $this->products_model->import_csv_balances($this->csv_path);
	...
	public function deductions($product_id,$store_id)
	{
		...
		//pagination
		$this->load->library('pagination');
		$config['base_url'] = site_url().'deductions/'.$product_id.'/'.$store_id;
		$config['total_rows'] = $this->users_model->count_items($table, $where);
		$config['uri_segment'] = $segment;
		$config['per_page'] = 20;
		$config['num_links'] = 5;
		...
		$query = $this->products_model->get_drugs_deductions($table, $where, $config["per_page"], $page, $order);
		...
		$v_data['title'] = $row->product_name.' Deductions';
		$data['content'] = $this->load->view('products/deductions', $v_data, true);
	
		$this->load->view('admin/templates/general_page', $data);
				
    }
    public function import_product_codes()
	{
		$v_data['title'] = $data['title'] = $this->site_model->display_page_title();
		
		$data['content'] = $this->load->view('import_product_codes', $v_data, true);
		$this->load->view('admin/templates/general_page', $data);
	}
	public function import_product_codes_template()
	{
		//export products template in excel 
		$this->products_model->import_product_codes_template();
	}
	public function do_product_codes_import()
	{
		if(isset($_FILES['import_csv']))
		{
			if(is_uploaded_file($_FILES['import_csv']['tmp_name']))
			{
				...
				$response = $this->products_model->import_csv_product_codes($this->csv_path);
	...
	public function product_sales($product_id)
	{
		...
		$v_data['title'] = $service_charge_name.' Sales';
		$v_data['all_categories'] = $this->categories_model->all_categories();
		$data['content'] = $this->load->view('products/product_sales', $v_data, true);
