...
{	
	...
	public function index()
	{
		...
		$constant  = ' AND product.store_id = store.store_id  AND store.store_status = 1 AND store.store_deleted = 0';
		$table = '';
		...
		if($is_admin OR $personnel_id == 6 OR $personnel_id == 12)
		{
			$where = 'product.category_id = category.category_id '.$constant.' AND product.product_deleted = 0 '.$added_items;
			$table = 'product, category, store,personnel_store';
		}
		else
		{
			$where = 'product.category_id = category.category_id '.$constant.' AND personnel_store.personnel_id = '.$personnel_id.' AND store.store_id = personnel_store.store_id AND product.product_deleted = 0 '.$added_items;
			$table = 'product, category, store,personnel_store';
	...
	*/
	...
	public function deduct_product($product_id)
	{
		...
		if ($this->form_validation->run())
		{
			...
			if($this->inventory_management_model->deduct_product($product_id))
			{
				...
				redirect('inventory/deduct-product/'.$product_id);
			}
    ...
    public function manage_store($store_id = 0, $orders_id = 0)
    {
...
//personnel_store.store_id  = store.store_id AND personnel_store.personnel_id = '.$this->session->userdata('personnel_id').''
		$where = 'orders.store_id = store.store_id AND personnel_store.store_id = store.store_id AND personnel_store.personnel_id = '.$this->session->userdata('personnel_id').' ';
		$table = 'orders, store, personnel_store';

		$order = 'orders.orders_date, store.store_name';
		// $where = 'store_product.store_id = product_deductions.store_id AND store_product.store_id = store.store_id AND product_deductions.product_id = product.product_id AND store_product.product_id = product.product_id ';
	...
	public function create_new_order()
	{
		...
		if ($this->form_validation->run())
		{
			...
			{
				$this->session->userdata('success_message', 'Product has been added successfully');
				redirect('inventory/manage-store');
			}
    ...
    public function store_requests($orders_id, $store_parent = 1)
    {
		...
		$where1 = 'product_deductions.order_id = orders.order_id AND orders.order_id = '.$orders_id.' AND product_deductions.store_id = store.store_id AND product_deductions.product_id = product.product_id';//echo $where;die();
	...
	public function search_supplier()
	{
		...
		if(!empty($order_id))
		{
			$orders = ' AND orders.order_id = \''.$order_id.'\'';
		}
		else
		{
			$order_id = '';
		...
		$search = $creditor_name.$category_id.$store_id.$supplier_invoice_date.$supplier_invoice_number;
		$this->session->set_userdata('search_supplier', $search);
		$this->index();
	}
	...
	public function make_order($store_id, $order_id)
	{
		...
		if($parent_id > 0)
		{
			$where = 'category.category_id = product.category_id AND product.product_id IN (SELECT product_id FROM store_product WHERE store_product.store_id = '.$parent_id.')';
		}
		
		else
		{
			$where = 'category.category_id = product.category_id AND product.product_id IN (SELECT product_id FROM store_product WHERE store_product.store_id = '.$store_id.')';
		}
		//echo $where; die();
	...
	public function costing($lab_test_id)
	{
		...
		if($store_priviledges->num_rows() > 0)
		{
			...
			foreach ($store_priviledges->result() as $key) 
			{
				...
				$store_parent = $key->store_parent;
				$store_id = $key->store_id;
				$constant = ' AND store_product.store_id = store.store_id AND product.product_id = store_product.product_id ';
		...
		$where = '(store.store_name = "Lab Store" OR store.store_name = "Lab Pharm") AND product.category_id = category.category_id '.$constant;
	...
	public function approve_request_order($order_id)
	{
		...
		redirect('inventory/product-deductions');
	}
