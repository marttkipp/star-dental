...
class Sync_model extends CI_Model 
{
	public function syn_up_on_closing_visit($visit_id)
	{
		...
		{
			$patient_details['mtiba_visit_id'] = $visit_id;
			$url =  $this->config->item('sync_url').'cloud/save_cloud_data';			
			//$test_url = 'https://www.omnis.co.ke/oserian-medical/test/cloud/test';
	...
	public function get_table_details($visit_id)
	{
		...
		if($table_sync_array->num_rows() > 0)
		{
			...
			foreach ($table_sync_array->result() as $key)
			{
				...
				{
					$where = 'visit.patient_id = patients.patient_id AND visit.visit_id = '.$visit_id;
					//echo ($where); die();
					$this->db->where($where);
	...
	public function save_diagnosis($diagnosis, $branch_code)
	{
		$res = $diagnosis;
		...
		//check if patient number exists
		$this->db->where('visit_diagnosis.visit_id = visit.visit_id AND payments.payment_id = '.$payment_id.' AND visit.branch_code = \''.$branch_code.'\'');
		$query = $this->db->get('payments, visit');
