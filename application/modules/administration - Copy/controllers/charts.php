...
class Charts extends auth
{
	...
	function patient_type_totals($timestamp)
	{
		...
		// $where = ' AND visit.branch_code = \'OSH\'';
		$where = ' AND visit.inpatient = 0';
		$total_outpatients = $this->reports_model->get_patient_type_total($where, $date);
		// $where = ' AND visit.branch_code = \'OSHB\'';
		$where = ' AND visit.inpatient = 1';
		$total_inpatients = $this->reports_model->get_patient_type_total($where, $date);
	...
	{	
		//get all service types
		$services_result = $this->reports_model->get_highest_drug_sales();
		...
		if($services_result->num_rows() > 0)
		{
			...
			foreach($result as $res)
			{
				$service_id = $res->product_id;
				$service_name = $res->service_charge_name;
	...
	{	
		//get all service types
		$services_result = $this->reports_model->get_highest_test_sales();
	...
	{	
		//get all service types
		$services_result = $this->reports_model->get_highest_objectives();
		...
		if($services_result->num_rows() > 0)
		{
			...
			foreach($result as $res)
			{
				$service_id = $res->objective_findings_id;
				$service_name = $res->objective_findings_name;
				$total = $res->total_sales;
	...
	{	
		//get all service types
		$services_result = $this->reports_model->get_highest_symptoms();
		...
		if($services_result->num_rows() > 0)
		{
			...
			foreach($result as $res)
			{
				$service_id = $res->symptoms_id;
				$service_name = $res->symptoms_name;
				$total = $res->total_sales;
