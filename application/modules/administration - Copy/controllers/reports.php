...
class Reports extends administration
{
	...
	public function doctor_patients_view($personnel_id,$payment_month=null,$payment_year = null,$week=null,$type=null)
	{
		...
		$where = 'visit.patient_id = patients.patient_id '.$add.' AND visit_type.visit_type_id = visit.visit_type AND visit.visit_delete = 0 AND visit.close_card <> 2 AND visit.personnel_id = '.$personnel_id;
		$table = 'visit, patients, visit_type';
		...
		{
			$where .=' AND MONTH(visit.visit_date) = "'.$payment_month.'" AND YEAR(visit.visit_date) ="'.$payment_year.'" '.$add_week.' ';

			$segment = 7;
			$config['base_url'] = site_url().'view-doctor-patients/'.$personnel_id.'/'.$payment_month.'/'.$payment_year.'/'.$week.'/'.$type;
		}
		else
		{
			$segment = 2;
	...
	public function debtors_report_invoices($visit_type_id, $order = 'debtor_invoice_created', $order_method = 'DESC')
	{
		...
		if($visit_type_id > 0)
		{
			...
			$where .= ' AND debtor_invoice.debtor_invoice_id = debtor_invoice_item.debtor_invoice_id';
	...
	public function create_new_batch($visit_type_id)
	{
		...
		$this->form_validation->set_rules('invoice_type', 'Type of Invoice', 'required|xss_clean');
	...
	{
		$where = 'debtor_invoice.debtor_invoice_id = '.$debtor_invoice_id.'';
		$table = 'debtor_invoice';
	...
	public function invoice($debtor_invoice_id)
	{
		...
		$type_where = 'debtor_invoice.debtor_invoice_id = '.$debtor_invoice_id.' AND visit_type.visit_type_id = debtor_invoice.visit_type_id';
		$type_table = 'debtor_invoice,visit_type';
	...
	public function cash_report()
	{
		...
		$table3 = 'payments';
		$where3 = 'payments.cancel = 0  AND payments.payment_method_id = 2';
	...
	public function all_invoices()
	{
		...
		$where = 'visit.patient_id = patients.patient_id AND visit_type.visit_type_id = visit.visit_type AND visit.visit_delete = 0  AND (visit.parent_visit = 0 OR visit.parent_visit IS NULL) AND preauth = 0';
		$table = 'visit, patients, visit_type';
		...
		$v_data['search'] = $visit_search;
		$v_data['total_patients'] = $config['total_rows'];
		$v_data['total_services_revenue'] = $this->reports_model->get_total_services_revenue_old($where, $table);
		$v_data['total_payments'] = $this->reports_model->get_total_cash_collection($where, $table);

		//total outpatients debt
		$where2 = $where.' AND patients.inpatient = 0';
		$total_outpatient_debt = $this->reports_model->get_total_services_revenue_old($where2, $table);
		//outpatient debit notes
		$where2 = $where.' AND payments.payment_type = 2 AND patients.inpatient = 0';
		$outpatient_debit_notes = $this->reports_model->get_total_cash_collection($where2, $table);
		//outpatient credit notes
		...
		//total inpatient debt
		$where2 = $where.' AND patients.inpatient = 1';
		$total_inpatient_debt = $this->reports_model->get_total_services_revenue_old($where2, $table);
		//inpatient debit notes
		$where2 = $where.' AND payments.payment_type = 2 AND patients.inpatient = 1';
		$inpatient_debit_notes = $this->reports_model->get_total_cash_collection($where2, $table);
		//inpatient credit notes
	...
	public function doctor_invoices()
	{
		...
		if($is_dentist AND $personnel_id <> 2)
		{
		...
		$where = 'visit.patient_id = patients.patient_id AND visit_type.visit_type_id = visit.visit_type AND visit.close_card <> 2 AND visit.visit_delete = 0  AND (visit.parent_visit = 0 OR visit.parent_visit IS NULL) '.$add;
		$table = 'visit, patients, visit_type';
	...
	public function search_invoices()
	{
		...
		if(!empty($invoice_number))
		{
			...
			$invoice_number = ' AND (visit.invoice_number = "'.$invoice_number.'" OR visit.visit_id = "'.$invoice_number.'") ';
		}
	...
	public function search_doctor_invoices()
	{
		...
		if(!empty($invoice_status))
		{
			...
			{
				$invoice_status_query = ' AND visit.visit_id IN (SELECT visit_id FROM doctor_invoice WHERE doctor_invoice_status = 1)';
			}
			else if($invoice_status == 2)
			{
				$invoice_status_query = ' AND visit.visit_id IN (SELECT visit_id FROM doctor_invoice WHERE doctor_invoice_status = 2)';
			}
		...
		$search = $invoice_number.$visit_type_id.$visit_date.$personnel_id.$other_name.$surname.$invoice_status_query;
	...
	public function invoice_hospital($visit_id,$type)
	{
		...
		if ($this->form_validation->run())
		{
			...
			if($this->reports_model->invoice_hospital($visit_id,$type))
			{
				$response['result'] ='success';
				$response['message'] ='You have successfully receipted the payment';
				// update visit to three
				$update_array['close_card'] = 5;
	...
	public function all_preauths()
	{
		...
		//pagination
		$this->load->library('pagination');
		$config['base_url'] = site_url().'preauths';
		$config['total_rows'] = $this->reception_model->count_items($table, $where);
		$config['uri_segment'] = $segment;
		$config['per_page'] = 20;
		$config['num_links'] = 5;
		...
		if(empty($page_title))
		{
			$page_title = 'Preauths for '.date('Y-m-d');
		}
		// var_dump($page_title);die();
		$data['title'] = $v_data['title'] = $page_title;
		$v_data['debtors'] = $this->session->userdata('debtors');
		...
		$data['content'] = $this->load->view('reports/all_preauths', $v_data, true);

		$this->load->view('admin/templates/general_page', $data);
	}

	public function search_preauths()
	{
		...
		$search = $invoice_number.$visit_type_id.$visit_date.$personnel_id.$patient_number.$surname;
		$this->session->unset_userdata('all_preauths_search');


		$this->session->set_userdata('all_preauths_search', $search);
		$this->session->set_userdata('search_title', $search_title);

		redirect('preauths');
	}

	public function close_preauth_search()
	{
		$this->session->unset_userdata('all_preauths_search');
	...
	public function export_preauths()
	{
		$this->reports_model->export_preauths();
	}
