...
{	
	...
	function upload_personnel_documents($personnel_id, $document)
	{
		$data = array(
			...
			'Description'=> $this->input->post('description_item'),
	...
	*
	*/
	public function retrieve_payroll_personnel2($where, $per_page, $page)
	{
		$this->db->where($where);
		$this->db->select('personnel.*, bank_branch.bank_branch_code');
		$this->db->group_by('personnel.personnel_id');
		$this->db->join('bank_branch', 'bank_branch.bank_branch_id = personnel.bank_branch_id', 'left');
		$query = $this->db->get('personnel, payroll_item', $per_page, $page);
	...
	*/
	...
	public function get_personnel_leave($personnel_id)
	{
		...
		$this->db->where('leave_duration.leave_type_id = leave_type.leave_type_id AND leave_duration.leave_duration_status = 1  AND YEAR(leave_duration.start_date)  = '.date('Y').' AND leave_duration.personnel_id = '.$personnel_id);
		$query = $this->db->get();
	...
	public function get_all_personnel_leaves($personnel_id)
	{
		//retrieve all users
		$this->db->from('leave_duration, leave_type');
		$this->db->select('leave_duration.*, leave_type.leave_type_name, leave_type.leave_type_count, leave_type.leave_days');
		$this->db->order_by('leave_type.leave_type_name');
		$this->db->where('leave_duration.leave_type_id = leave_type.leave_type_id  AND YEAR(leave_duration.start_date)  = '.date('Y').' AND leave_duration.personnel_id = '.$personnel_id);
		$query = $this->db->get();
	...
	*/
		...
		if(($total_rows > 0) && ($total_columns == 17))
		{
			...
			for($r = 1; $r < $total_rows; $r++)
			{
				...
				if(!empty($personnel_number))
				{
					...
					if(($this->check_current_personnel_exisits($personnel_number, $branch_id))== TRUE)
					{
	...
	{
		$approval_id = $this->input->post('approval_role_id');
		$data = array(
				'personnel_id' => $personnel_id,
	...
	public function personnel_export($order = 'branch_id', $order_method = 'ASC')
	{
		if($this->load->library('excel'))
		...
		$query = $this->personnel_model->get_all_download_personnel($table, $where);
		
		$title = 'Personnel Bio data '.date('jS M Y H:i a',strtotime(date('Y-m-d H:i:s')));
		$col_count = 0;
		...
		{//var_dump($query);die();
			...
			$col_count++;
			$report[$row_count][$col_count] = 'Personnel Number';
			$col_count++;
			$report[$row_count][$col_count] = 'Personnel Name';
			$col_count++;
			$report[$row_count][$col_count] = 'Branch Name';
			$col_count++;
			$report[$row_count][$col_count] = 'KRA Pin';
			$col_count++;
			$report[$row_count][$col_count] = 'NSSF No.';
			$col_count++;
			$report[$row_count][$col_count] = 'NHIF No.';
			$col_count++;
			$report[$row_count][$col_count] = 'Email Address';
			$col_count++;
			$report[$row_count][$col_count] = 'Bank Account No.';
			$col_count++;
			$report[$row_count][$col_count] = 'Bank Code';
			$col_count++;
			$report[$row_count][$col_count] = 'Personnel Status';
			$col_count++;
			...
			{//var_dump($query);die();
				...
				{//var_dump($bank_branch_id);die();
					$bank_branch_code = $this->payroll_model->get_bank_branch_code($bank_branch_id, $personnel_id);
					
					if(empty($bank_branch_code))
					{
						$bank_branch_code = $bank_branch_id;
					}
				}
				else
				{
				...
				$col_count++;
				$report[$row_count][$col_count] = $personnel_number;
				$col_count++;
				$report[$row_count][$col_count] = $personnel_name;
				$col_count++;
				$report[$row_count][$col_count] = $branch;
				$col_count++;
				$report[$row_count][$col_count] = $personnel_kra_pin;
				$col_count++;
				$report[$row_count][$col_count] = $personnel_nssf_number;
				$col_count++;
				$report[$row_count][$col_count] = $personnel_nhif_number;
				$col_count++;
				$report[$row_count][$col_count] = $personnel_email;
				$col_count++;
				$report[$row_count][$col_count] = sprintf('"%014d',$bank_account_number);
				$col_count++;
				$report[$row_count][$col_count] = sprintf('"%05d', $bank_branch_code);
				$col_count++;
	...
	public function get_all_download_personnel($table, $where)
	{
		$this->db->select('*');
		$this->db->where($where);
	...
	public function check_pending_leave($personnel_id)
	{
		//retrieve all users
		$this->db->from('leave_duration, leave_type');
		$this->db->select('leave_duration.*, leave_type.leave_type_name, leave_type.leave_type_count, leave_type.leave_days');
		$this->db->order_by('leave_type.leave_type_name');
		$this->db->where('leave_duration.leave_type_id = leave_type.leave_type_id AND leave_duration.leave_duration_status = 0 AND YEAR(leave_duration.start_date)  = '.date('Y').' AND leave_duration.personnel_id = '.$personnel_id);
		$query = $this->db->get();
