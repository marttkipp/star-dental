...
class Personnel extends hr 
{
	...
	*/
	public function add_personnel() 
	{
		...
		$this->form_validation->set_rules('personnel_kra_pin', 'KRA PIN', 'xss_clean|is_unique[personnel.personnel_kra_pin]');
		$this->form_validation->set_rules('personnel_national_id_number', 'National ID', 'xss_clean|is_unique[personnel.personnel_national_id_number]');
		$this->form_validation->set_rules('bank_account_number', 'Bank Account', 'xss_clean');
		$this->form_validation->set_rules('bank_branch_id', 'Bank Branch', 'xss_clean');
		$this->form_validation->set_rules('bank_id','Bank Name','xss_clean');
		$this->form_validation->set_rules('bank_code','Bank','xss_clean');
		...
		$v_data['bank_names'] = $this->personnel_model->get_bank_names();
		$v_data['bank_branch_names'] = $this->personnel_model->get_bank_branch_names();
	...
	*/
    ...
    public function update_personnel_about_details($personnel_id)
    {
		...
		$this->form_validation->set_rules('personnel_city', 'City', 'xss_clean');
		$this->form_validation->set_rules('personnel_post_code', 'Post code', 'xss_clean');
		$this->form_validation->set_rules('bank_account_number', 'Account number', 'xss_clean');
		...
		$this->form_validation->set_rules('personnel_type_id', 'Personnel_type', 'required|xss_clean');
		$this->form_validation->set_rules('bank_id', 'Bank', 'xss_clean');
		$this->form_validation->set_rules('bank_code', 'Bank', 'xss_clean');
		...
		if ($this->form_validation->run())
		{
			...
			else
			{
				...
				if(!empty($image_error))
				{
					$error .= '<br/><strong>Image upload error!</strong> '.$image_error;
				}
	...
	*/
	public function edit_about($personnel_id) 
	{
		...
		$this->form_validation->set_rules('bank_branch_id', 'Branch Name', 'xss_clean');
		$this->form_validation->set_rules('personnel_account_number', 'Account Number', 'xss_clean');
		$this->form_validation->set_rules('bank_id', 'Bank Name', 'xss_clean');
		$this->form_validation->set_rules('bank_code', 'Bank_code', 'xss_clean');
	...
	*/
	public function upload_documents($personnel_id) 
	{
		...
		$this->form_validation->set_rules('description_item', 'Description', 'required|xss_clean');
	...
	public function import_personnel_emails()
	{
		$v_data['title'] = $data['title'] = $this->site_model->display_page_title();
		
		$data['content'] = $this->load->view('import/import_personnel_emails', $v_data, true);
		$this->load->view('admin/templates/general_page', $data);
	}
	public function import_personnel_emails_template()
	{
		//export products template in excel 
		$this->personnel_model->import_personnel_emails_template();
	}
	
	public function do_personnel_emails_import()
	{
		if(isset($_FILES['import_csv']))
		{
			if(is_uploaded_file($_FILES['import_csv']['tmp_name']))
			{
				//import products from excel 
				$response = $this->personnel_model->import_csv_personnel_emails($this->csv_path);
	...
	public function delete_personnel_approvals_assigned($personnel_approval_id, $personnel_id)
    {
		if($this->personnel_model->delete_personnel_approvals_assigned($personnel_approval_id))
		{
			$this->session->set_userdata("success_message", "Approval deleted successfully");
			redirect('human-resource/edit-personnel/'.$personnel_id);
		}
		
		else
		{
			$this->session->set_userdata("error_message","Could not delete  the assigned approval. Please try again");
			redirect('human-resource/edit-personnel/'.$personnel_id);
		}
    }
	public function add_personnel_timesheet()
	{
		...
		$data['content'] = $this->load->view('personnel/add_timesheet', $v_data, true);
