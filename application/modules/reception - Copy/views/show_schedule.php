...
echo '<h5 style="font-family:Palatino Linotype;">Appointment List from '.$date.' </h5>';

echo 'No appointments scheduled on '.$date;
}
else{
$rs1= $this->reception_model->doctors_names($personnel_id);
if (count($rs1) > 0)
...
echo '<tr> <td>'.$visit_date.' </td> <td>'.$patient_surname.' '.$patient_othernames.'</td> <td>'.$time_start.' <td>'.$time_end.'</td> <td>Dr. '.$drname2.'</td>  </tr> ';
