<!-- search -->
<?php echo $this->load->view('search_deleted_invoices', '', TRUE);?>
<!-- end search -->
...
<?php
		$result = '<a href="'.site_url().'accounting/reports/export_deleted_invoices" target="_blank" class="btn btn-sm btn-success pull-right">Export</a>';
		$search = $this->session->userdata('visit_deleted_search_query');
		if(!empty($search))
		{
			echo '<a href="'.site_url().'accounting/reports/close_deleted_reports_search" class="btn btn-sm btn-warning">Close Search</a>';
		}
		...
		if ($query->num_rows() > 0)
		{
			...
			$result .= 
				'
					<table class="table table-hover table-condensed table-bordered table-striped table-responsive col-md-12">
					  <thead>
						<tr>
						  <th>#</th>
						  <th>Visit Date</th>
