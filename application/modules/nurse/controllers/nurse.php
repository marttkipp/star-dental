...
{	
	...
	*/
	...
	public function close_patient_treatment_search()
	{
		$this->session->unset_userdata('patient_treatment_statement_search');
		$this->patient_treatment_statement();
	}
	public function patient_treatment_statement()
	{
		$module = NULL;
		$segment = 3;
		...
		//pagination
		$this->load->library('pagination');
		$config['base_url'] = site_url().'patient-treatment';
		$config['total_rows'] = $this->reception_model->count_items($table, $where);
		$config['uri_segment'] = $segment;
		$config['per_page'] = 20;
		$config['num_links'] = 5;
		...
		$page = ($this->uri->segment($segment)) ? $this->uri->segment($segment) : 0;
        $v_data["links"] = $this->pagination->create_links();
		$query = $this->reception_model->get_all_patients_visit($table, $where, $config["per_page"], $page);
	...
	public function search_patient_treatment_statement($module)
	{
		...
		if(!empty($_POST['surname']))
		{
			...
			$search = $patient_number.$surname.$visit_date;
			$this->session->set_userdata('patient_treatment_statement_search', $search);
			
			redirect('patient-treatment');
	}
	public function close_treatment_search()
	{
		$this->session->unset_userdata('patient_treatment_statement_search');
		redirect('dental/patient-treatment');
	}
	public function treatment_statement($patient_id,$module = NULL)
	{
		$segment = 5;		// this is it

		$where = 'visit.patient_id = patients.patient_id AND visit.close_card <> 2 AND (visit.parent_visit = 0 OR visit.parent_visit IS NULL) AND visit.visit_delete = 0 AND visit.dental_visit = 0  AND visit.`patient_id`='.$patient_id;
